...
void CCameraComponent::MaitrxCalculate()
{
	...
	D3DXMatrixRotationYawPitchRoll(&rotationMatrix, D3DXToRadian(GetOwner()->GetRotation().y), D3DXToRadian(GetOwner()->GetRotation().x), D3DXToRadian(GetOwner()->GetRotation().z));
	...
	D3DXMatrixPerspectiveFovLH(&m_matProj, D3DXToRadian(m_fov), 800.0f / 600.0f, m_near, m_far);
	D3DXMatrixOrthoLH(&m_matOrtho, 800.0f, 600.0f, 0, m_far);
}
...
_bool CCameraComponent::IsInFrustum(vector3 & position)
{
	/*
