...
{
protected:
	GETTOR_SETTOR(matrix4x4, m_matView,{}, ViewMatrix)
	GETTOR_SETTOR(matrix4x4, m_matOrtho,{}, OrthoMatrix)
	GETTOR_SETTOR(matrix4x4, m_matProj,{}, ProjMatrix)

		// 카메라가 바라보는곳 , 카메라의 상방 벡터
	GETTOR_SETTOR(vector3, m_at,{}, At)
	GETTOR_SETTOR(vector3, m_up,{}, Up)
	...
	GETTOR_SETTOR(vector3, m_tAt, vector3Zero, TempAt)
	GETTOR_SETTOR(vector3, m_tUp, vector3Zero, TempUp)
	...
	GETTOR_SETTOR(_bool, m_isCameraEvent, false, CameraEvent)
		...
		// FREAM마다 마우으의 위치 변화값
	GETTOR(vector2, m_mouseDelta,{}, MouseDelta);
